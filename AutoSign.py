...
def push(SENDKEY, res, QQSENDKEY, TGCHATID, BOTTOKEN):
    ...
    if QQSENDKEY == '':
        ...
        if res.text == 'success':
            ...
            rQmsgchan = requests.post('https://qmsg.zendee.cn:443/send/{sendkey}'.format(sendkey=QQSENDKEY),
                                      data={'msg': "学习通：" + course_dict[currClass][0] + "签到成功"})
        elif res.text == '您已签到过了':
            flag2 = 1
            # rQmsgchan = requests.post('https://qmsg.zendee.cn:443/send/{sendkey}'.format(sendkey=QQSENDKEY),
            #                           data={'msg': "学习通：" + course_dict[currClass][0] + "您已签到过了"})
        else:
            rQmsgchan = requests.post('https://qmsg.zendee.cn:443/send/{sendkey}'.format(sendkey=QQSENDKEY),
                                      data={'msg': "学习通：" + "签到失败，原因：" + res.text})
        if flag2 == 1:
    ...
    if TGCHATID == '' or BOTTOKEN == '':
        print("TGCHATID 或 BOTTOKEN 参数为空，跳过 Telegram 推送")
    else:
        if res.text == 'success':
            # Telegram 推送
